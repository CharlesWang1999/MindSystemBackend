...
function initPlayer(){
    ...
    disabledSubmitButton(next_page,"心情平复后可以开始下一章节");
    selectTrack(0);
    // 绑定播放暂停按钮的点击事件
    ...
    next_page.addEventListener('click',back_the_question);
    //跳转至下一个问题界面
}
...
function selectTrack(flag){
    ...
    if(cur_index>-1 && cur_index<albums.length){
        ...
        // 设置音频路径
        audio.src='/static/mp3/'+cur_album+'.mp3';
        if(flag!=0){
            // 当切换上一首,下一首时,自动播放
            audio.play();
            player_track.classList.add('active');
 ...
 function disabledSubmitButton(submitButton, submitButtonText) {
    ...
    var intervalObj = setInterval(function () {
        ...
        if (second == 0) {
            ...
            submitButton.removeAttribute("disabled");//将按钮可用
            /* 清除已设置的setInterval对象 */
    ...
    $.ajax({
      ...
      success:function (response) {
        ...
        if(response['next_page_round'] == 's4' ) {
            url = "/ARPicture/eq_evaluate/"+ response['uaid'] + '/'
          }
